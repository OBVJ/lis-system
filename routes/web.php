...
// Temporary route for seeding users
    ...
    try {
        ...
        if (!in_array('users', $tableNames)) {
            return response()->json([
                'success' => false,
                'message' => 'Users table does not exist. Please run migrations first.'
            ], 400);
        }
        ...
        return response()->json([
            ...
            'credentials' => [
                'admin' => ['email' => 'admin@lis.com', 'password' => '123456'],
                'doctor' => ['email' => 'doctor@lis.com', 'password' => '123456'],
                'receptionist' => ['email' => 'receptionist@lis.com', 'password' => '123456'],
                'technician' => ['email' => 'technician@lis.com', 'password' => '123456'],
                'test_users' => 'test1@lis.com to test4@lis.com (password: 123456)'
            ]
        ]);
    } catch (\Exception $e) {
        return response()->json([
...
// Professional Receipt Routes
Route::get('/receipt/{labRequest}', [ReportController::class, 'receipt'])->name('receipt.print');
Route::get('/receipt/{labRequest}/thermal', [ReportController::class, 'thermalReceipt'])->name('receipt.thermal');
...
// Temporary route for clearing cache
...
// Temporary route for resetting migrations
...
// Temporary route for truncating migrations table
Route::get('/truncate-migrations', function () {
    try {
        \Illuminate\Support\Facades\DB::table('migrations')->truncate();

        return response()->json([
            'success' => true,
            'message' => 'Migrations table truncated successfully!'
        ]);
    } catch (\Exception $e) {
        return response()->json([
            'success' => false,
            'message' => 'Error truncating migrations table: ' . $e->getMessage()
        ], 500);
    }
});

// Temporary route for listing migration files
Route::get('/list-migrations', function () {
    try {
        ...
        return response()->json([
            'success' => true,
            'migration_files' => array_keys($files),
            'total' => count($files)
        ]);
    } catch (\Exception $e) {
        return response()->json([
            'success' => false,
            'message' => 'Error listing migrations: ' . $e->getMessage()
        ], 500);
    }
});

// Test route to check locale
